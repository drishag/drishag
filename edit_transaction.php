...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($_SESSION['transactions'] as &$t) {
        if ($t['id'] == $id) {
            ...
            $t['items'] = $items;
            $t['net_amount'] = $totalAmount - ($totalAmount * VAT_RATE / (1 + VAT_RATE));
            $t['vat_amount'] = $totalAmount * VAT_RATE / (1 + VAT_RATE);
            $t['total_amount'] = $totalAmount;
            break;
        }
    }
    
    header('Location: transactions.php?updated=' . $id);
    exit;
}
?>
    ...
    <div class="col-md-10">
        <div class="card vat-card">
            <div class="card-header bg-primary text-white">
                <h4><i class="bi bi-pencil"></i> تعديل فاتورة #<?= $transaction['invoice_number'] ?></h4>
            </div>
            <div class="card-body">
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <select class="form-select" id="type" name="type" required>
                                <option value="<?= TRANSACTION_TYPE_SALE ?>" <?= $transaction['type'] == TRANSACTION_TYPE_SALE ? 'selected' : '' ?>>فاتورة مبيعات</option>
                                <option value="<?= TRANSACTION_TYPE_PURCHASE ?>" <?= $transaction['type'] == TRANSACTION_TYPE_PURCHASE ? 'selected' : '' ?>>فاتورة مشتريات</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label for="date" class="form-label">تاريخ الفاتورة</label>
                            <input type="date" class="form-control" id="date" name="date" 
                                   value="<?= date('Y-m-d', strtotime($transaction['date'])) ?>" required>
                        </div>
                    </div>
                    ...
                    <div id="itemsContainer">
                        ...
                        <div class="item-row row mb-3 border-bottom pb-3">
                            ...
                            <div class="col-md-2">
                                ...
                                <input type="number" class="form-control item-quantity" name="item_quantity[]" step="0.01" min="0" value="<?= $item['quantity'] ?>" required>
                            </div>
                            <div class="col-md-2">
                                <label class="form-label">السعر</label>
                                <input type="number" class="form-control item-price" name="item_price[]" step="0.01" min="0" value="<?= $item['price'] ?>" required>
                            </div>
                            <div class="col-md-2">
                                <label class="form-label">الضريبة</label>
                                <div class="form-check mt-2">
                                    <input class="form-check-input item-tax" type="checkbox" name="item_tax_included[]" <?= $item['is_tax_included'] ? 'checked' : '' ?>>
                                ...
                                <div class="col-md-6 offset-md-6">
                                    <table class="table table-bordered">
                                        <tr>
                                            ...
                                            <td id="subtotal"><?= number_format($transaction['net_amount'], 2) ?> ر.س</td>
                                        </tr>
                                        <tr>
                                            <th>الضريبة (<?= (VAT_RATE * 100) ?>%):</th>
                                            <td id="vatAmount"><?= number_format($transaction['vat_amount'], 2) ?> ر.س</td>
                                        </tr>
                                        <tr class="table-primary fw-bold">
                                            <th>الإجمالي النهائي:</th>
                                            <td id="totalAmount"><?= number_format($transaction['total_amount'], 2) ?> ر.س</td>
                                        </tr>
                                    </table>
                                </div>
                            </div>
